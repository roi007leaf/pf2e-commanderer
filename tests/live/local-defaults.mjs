...
export const defaultsPath = path.join(homedir(), '.config', 'pf2e-commanderer', 'live.json');

export async function loadLocalDefaults(file = defaultsPath) {
  let text;
  try { text = await readFile(file, 'utf8'); }
  catch (error) { if (error.code === 'ENOENT') return {}; throw Error('Cannot read local live-test defaults', { cause: error }); }
  try {
    const value = JSON.parse(text);
    if (!value || typeof value !== 'object' || Array.isArray(value)) throw Error();
    if (value.url !== undefined && typeof value.url !== 'string') throw Error();
    if (value.world !== undefined && (typeof value.world !== 'string' || !value.world.trim())) throw Error();
...
export function accountDefaults(role, local = {}, env = process.env) {
  const prefix = `COMMANDERER_${role.toUpperCase()}`;
  ...
  const allowBlankPassword = role === 'player' && (env.COMMANDERER_PLAYER_ALLOW_BLANK !== undefined
    ? env.COMMANDERER_PLAYER_ALLOW_BLANK === '1' : sameAccount && saved.allowBlankPassword === true);
...
export async function promptAccount(role, local, env, prompts, context) {
  ...
  if (secret === undefined && role === 'player' && env.COMMANDERER_PLAYER_ALLOW_BLANK === '1') secret = '';
  if (secret === undefined) {
  ...
  const allowBlankPassword = role === 'player' && !secret && (env.COMMANDERER_PLAYER_ALLOW_BLANK === '1' ||
