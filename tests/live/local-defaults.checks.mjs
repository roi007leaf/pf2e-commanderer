...
test('explicit environment credentials retain unattended operation', async () => {
  ...
  assert.equal((await promptAccount('gm', saved, { COMMANDERER_GM_USER: 'Env GM', COMMANDERER_GM_PASSWORD: 'env-secret' }, prompts)).password, 'env-secret');
  assert.deepEqual(await promptAccount('player', saved, { COMMANDERER_PLAYER_USER: 'Env Player', COMMANDERER_PLAYER_ALLOW_BLANK: '1' }, prompts), {
...
test('environment overrides defaults without reusing another account password', () => {
  assert.equal(accountDefaults('gm', saved, { COMMANDERER_GM_PASSWORD: 'new-secret' }).password, 'new-secret');
  assert.equal(accountDefaults('gm', saved, { COMMANDERER_GM_USER: 'Other GM' }).password, undefined);
  assert.equal(accountDefaults('player', saved, { COMMANDERER_PLAYER_USER: 'Other Player' }).allowBlankPassword, false);
  assert.equal(accountDefaults('player', saved, { COMMANDERER_PLAYER_ALLOW_BLANK: '0' }).allowBlankPassword, false);
});
...
test('missing defaults still prompt; malformed defaults never expose their contents', async () => {
  ...
  try {
    ...
    await writeFile(file, JSON.stringify({ ...saved, world: 'visioner-qa' }));
    assert.equal((await loadLocalDefaults(file)).world, 'visioner-qa');
